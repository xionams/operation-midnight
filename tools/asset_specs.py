...
def tracks(mesh, width, length, height=0.45, inset=0.08, road_wheels=4):
    """Running gear: track band, road wheels, sprocket, idler and fender.

    A tracked vehicle is recognised by its RUNNING GEAR before anything
    else - it is the row of wheels and the fender line that say "tank"
    rather than "box on the ground". The first version of this was two
    dark boxes, which grounded the hull and said nothing else.

    The road wheels sit slightly proud of the band on purpose, so the sun
    picks out a row of highlights along the side. That row is the whole
    effect: at 40m the individual wheels are not resolvable, but the
    rhythm of light along the hull side is.
    """
    half = width / 2.0 - width * 0.13 - inset
    band = width * 0.26
    radius = height * 0.42
    for side in (-1, 1):
        x = side * half
        box(mesh, "Dark", (band, height, length), (x, height / 2.0, 0.0),
            chamfer=0.06)
        ## Four wheels, six segments. Five was tried and the fifth is not
        ## distinguishable at gameplay zoom - what the eye reads is the
        ## rhythm, not the count.
        span = length * 0.74
        for i in range(road_wheels):
            z = (i - (road_wheels - 1) / 2.0) * (span / max(road_wheels - 1, 1))
            cylinder(mesh, "Metal", radius, band * 0.58, (x, radius * 1.04, z),
                     segments=6, axis="x")
        ## Drive sprocket and idler, larger and at the extremes - the
        ## asymmetry of big-small-small-small-big is what reads as a track
        ## run rather than as a row of identical discs.
        for z in (-length * 0.45, length * 0.45):
            cylinder(mesh, "Metal", radius * 1.18, band * 0.64,
                     (x, radius * 1.06, z), segments=6, axis="x")
        ## Fender overhanging the track. Casts a shadow line down the side
        ## and hides where the band meets the hull.
        box(mesh, "Hull", (band * 1.3, 0.08, length * 0.95),
            (x, height + 0.04, 0.0), chamfer=0.03)


def wheels(mesh, width, length, radius=0.34, pairs=2):
    """Wheeled running gear: dark tyre with a lighter hub proud of it."""
    for side in (-1, 1):
        for i in range(pairs):
            z = (i - (pairs - 1) / 2.0) * (length * 0.52)
            x = side * (width / 2.0 - width * 0.08)
            cylinder(mesh, "Dark", radius, width * 0.16, (x, radius, z),
                     segments=8, axis="x")
            ## Without the hub a wheel is a black disc and reads as a hole
            ## in the vehicle rather than as a wheel.
            cylinder(mesh, "Metal", radius * 0.44, width * 0.19,
                     (x, radius, z), segments=6, axis="x")


def stowage(mesh, cx, cy, cz, scale=1.0):
    """Crates and a rolled tarp strapped to the hull.

    Clutter is what separates a vehicle in service from a showroom model,
    and it breaks the straight line of a hull deck from above - which is
    the view this game is played from.
    """
    box(mesh, "Sand", (0.42 * scale, 0.24 * scale, 0.30 * scale),
        (cx, cy + 0.12 * scale, cz), chamfer=0.03)
    box(mesh, "Dark", (0.30 * scale, 0.20 * scale, 0.26 * scale),
        (cx - 0.34 * scale, cy + 0.10 * scale, cz + 0.06 * scale), chamfer=0.03)
    cylinder(mesh, "Olive", 0.11 * scale, 0.52 * scale,
             (cx + 0.30 * scale, cy + 0.11 * scale, cz), segments=6, axis="x")
...
def _assault_vehicle():
    ...
    stowage(m, -w * 0.16, 1.03, d * 0.30, scale=0.85)
    turret = Mesh()
...
def _main_battle_tank():
    ...
    ## Kit on the back deck. A bare deck plate reads as unused.
    stowage(m, 0.0, 1.05, d * 0.34, scale=1.1)
    turret = Mesh()
    box(turret, "Hull", (1.55, 0.52, 1.75), (0, 0, 0.05), chamfer=0.14)
    wedge(turret, "Hull", (1.55, 0.26, 0.70), (0, 0.39, -0.55))
    ## Mantlet. The step from turret face to barrel is what stops the gun
    ## looking like a stick pushed into a box.
    box(turret, "Dark", (0.62, 0.44, 0.34), (0, 0.02, -0.92), chamfer=0.08)
    ...
    ## Commander's cupola, offset. Breaks the turret's symmetry, which is
    ## what tells the player which way it is facing when the gun is
    ## pointing at the camera.
    cylinder(turret, "Hull", 0.30, 0.22, (-0.40, 0.37, 0.34), segments=8)
    cylinder(turret, "Dark", 0.09, 0.44, (-0.40, 0.58, 0.18), segments=6, axis="z")
